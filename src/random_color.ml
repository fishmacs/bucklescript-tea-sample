...
type model = { r: int; g: int; b: int }

type msg = Roll | NewColor of int * int * int

let init () = { r = 255; g = 255; b = 255}, Cmd.none

let update model = function
  | Roll -> let genInt = Tea.Random.int 1 255 in
...
let view model =
  ...
  let rgb = Printf.sprintf "(%d, %d, %d)" model.r model.g model.b in
  let altRgb = if isDark then "(0, 0, 0)" else "(255, 255, 255)" in
  div
    []
    [ h1
        [ style "background-color" ("rgb" ^ rgb); style "color" ("rgb" ^ altRgb) ]
        [ text rgb]
