...
let update model = function
  | Add -> {
      ...
      entries =
        ...
        else newEntry model.field model.uid :: model.entries
    }, Cmd.none
...
let viewEntries visibility entries =
  ...
  in section
    ...
    ; ul
        [ class' "todo-list" ]
        (List.rev_map
           (fun todo -> lazy1 (string_of_int todo.id ^ string_of_bool todo.completed ^ string_of_bool todo.editing) (viewEntry todo))
...
let view model =
  div
    ...
    [ section
        ...
        (* Optimization: Set the input as lazy field *)
