...
type msg = UpdateValue of string | UpdateTickTime of string | DoUpdate of Time.t
...
let update model = function
  ...
  | DoUpdate _ ->
    let data: int array array =
      ...
      let () = data.(halfX).(halfY) <- data.(halfX).(halfY) + model.value in
      let () =
    ...
    let maxValue =
      let inner value arr =
        Array.fold_left
          (fun maxValue value -> if maxValue > value then maxValue else value)
          value
          arr in
      Array.fold_left inner 1 data in {
      model with data; maxValue
    }, Cmd.none
...
let view model =
  ...
  let mapperY coordX coordY value =
    let colorHex =
      let red =
        if value > 2000000 then "FF"
        else if value > 1000000 then "40"
        else "00" in
      let green =
        if value > 750000 then "FF"
        else if value > 550000 then "80"
        else "00" in
      let blue =
  ...
  ; Html.br []
  ; svg
      [ viewBox "0 0 300 300"; width "300px" ]
