...
type msg = Increment | Decrement | Reset | Set of int | OnUrlChange of int option
...
let init () location =
  let open Web.Location in match fromUrl location.hash with
  | None -> 0, Navigation.modifyUrl (toUrl 0)
  | Some v -> v, Cmd.none

let main = let open Navigation in
