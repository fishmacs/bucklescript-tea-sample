...
type objectTypes = Base of int * int
...
let fromUrl url =
  let mapDecoder = let open Json.Decoder in
    array (array int) in
  match Json.Decoder.decodeString mapDecoder url with
  | Result.Ok map -> map
  | Result.Error _ -> Array.make_matrix 32 32 0
...
let init () location =
  ...
  let units =
    IntPairMap.empty
  |> IntPairMap.add (4, height - 4) [Base (1, 12)]
  |> IntPairMap.add (width - 4, 4) [Base (-1, 12)]
  in
  let model = {
    map;
...
let rec mergeUnits dict = function
  | [] -> dict
  | (coords, unit) :: rest ->
    addUnit coords unit (mergeUnits dict rest)
  ...
  | AITick ->
    ...
    let tickUnit (x, y) ret = function
      | Base (team, inf) ->
        ...
        if x > 1 && y > 1 && influence.(x-1).(y-1) * team > 2 &&
           false = (IntPairMap.mem (x-1, y-1) units)
        then ((x-1, y-1), newBase) :: ret
        else if x > 1 && y < (height-2) && influence.(x-1).(y+1) * team > 2 &&
           false = (IntPairMap.mem (x-1, y+1) units)
        then ((x-1, y+1), newBase) :: ret
        else if x < (width-2) && y > 1 && influence.(x+1).(y-1) * team > 2 &&
           false = (IntPairMap.mem (x+1, y-1) units)
        then ((x+1, y-1), newBase) :: ret
        else if x < (width-2) && y < (height-2) && influence.(x+1).(y+1) * team > 2 &&
           false = (IntPairMap.mem (x+1, y+1) units)
        then ((x+1, y+1), newBase) :: ret
        else ret in
  ...
  | InfluenceTick ->
    ...
    let processInfY x y t =
      if x = 0 || x = width - 1 || y = 0 || y = height - 1 then
        ...
        let part = inf / 6 in
        let () = influence.(x-1).(y) <- infXN + part in
        let () = influence.(x+1).(y) <- infXP + part in
        let () = influence.(x).(y-1) <- infYN + part in
        let () = influence.(x).(y+1) <- infYP + part in
        influence.(x).(y) <- inf - (part * 4)
      else () in
    let processX x yArr = Array.iteri (processInfY x) yArr in
    let reduceInfY x y i =
      if i < -255 then influence.(x).(y) <- -255
      else if i < 0 then influence.(x).(y) <- i + 1
      else if i > 255 then influence.(x).(y) <- 255
      else if i > 0 then influence.(x).(y) <- i - 1
      else () in
    let reduceInfX x yArr = Array.iteri (reduceInfY x) yArr in
    ...
    let (scoreN, scoreP) =
      ...
      let scoreX scores yArr = Array.fold_left scoreY scores yArr in
...
let subscriptions _ =
  Sub.batch [
    Time.every (Time.inMilliseconds 500.) (fun _ -> InfluenceTick);
    Time.every (Time.inMilliseconds 1100.) (fun _ -> AITick)
  ]
...
let view model =
  ...
  let unitsSvg () =
    ...
    IntPairMap.bindings model.units |> List.map unitToSvg |> List.flatten in
  let tileToSvg cx cy value =
    let key = string_of_int value in
    if cx = 0 || cx = mapWidth - 1 || cy = 0 || cy = (mapHeight - 1) then noNode
    else if value = 0 then
      let v = model.influence.(cx).(cy) in
      let fillColor =
        let av = abs(v) in
        let density = if av >= maxInfluence then 0 else 255 - av in
        let color =
          if v < 0 then Printf.sprintf "rgb(255,%d,%d)" density density
          else if v > 0 then Printf.sprintf "rgb(%d,%d,255)" density density
  ...
  Html.div [] [
    svg [
      viewBox (Printf.sprintf "0 0 %d %d" (mapWidth * cellSize) (mapHeight * cellSize));
      width (Printf.sprintf "%dpx" (mapWidth * cellSize))
    ] [
